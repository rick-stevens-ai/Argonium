...
class MasterExtractor:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            self.console.print(
                f"✅ Initialized client for {self.config.model_config.shortname}"
            )
    ...
    def _read_single_file(self, file_path: str) -> str:
        ...
        try:
            ...
            if len(content) > self.config.character_limit:
                ...
                self.console.print(
                    f"⚠️  Text truncated to {self.config.character_limit} characters for {file_path}"
                )
    ...
    def _get_prompt_for_mode(
        self, mode: ExtractionMode, chunk_text: str, chunk_idx: int, total_chunks: int
    ) -> str:
        ...
        if mode == ExtractionMode.WORKFLOW:
            ...
            return self._get_workflow_calculus_prompt(
            ...
            return self._get_experimental_protocol_prompt(
                chunk_text, chunk_idx, total_chunks
            )
        elif mode == ExtractionMode.AUTOMATED_PROTOCOL:
            return self._get_automated_protocol_prompt(
                chunk_text, chunk_idx, total_chunks
            )
        else:
    ...
    def _get_workflow_prompt(
        self, chunk_text: str, chunk_idx: int, total_chunks: int
    ) -> str:
...
{chunk_text}"""
    ...
    def _get_problem_summary_prompt(
        self, chunk_text: str, chunk_idx: int, total_chunks: int
    ) -> str:
...
{chunk_text}"""
    ...
    def _get_workflow_calculus_prompt(
        self, chunk_text: str, chunk_idx: int, total_chunks: int
    ) -> str:
        """Generate workflow calculus prompt (from l33_process_summary_to_workflow.py)."""
        notation_guidance = self.guidance.get(
            "notation", "[Notation guidance not available]"
        )
...
{chunk_text}"""
    ...
    def _get_experimental_protocol_prompt(
        self, chunk_text: str, chunk_idx: int, total_chunks: int
    ) -> str:
...
{chunk_text}"""
    ...
    def _get_automated_protocol_prompt(
        self, chunk_text: str, chunk_idx: int, total_chunks: int
    ) -> str:
...
{chunk_text}"""
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a helpful assistant that analyzes scientific texts for bioinformatics workflows.",
                    },
    ...
    def _process_chunk_parallel(
        self, chunk_data: Tuple[int, str, int, int]
    ) -> Tuple[int, str]:
        ...
        try:
            ...
            prompt = self._get_prompt_for_mode(
                self.config.mode, chunk_text, chunk_num, total_chunks
            )
...
{sample_text}"""
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a helpful assistant that classifies scientific texts.",
                    },
                    {"role": "user", "content": prompt},
                ],
                max_tokens=10,
    ...
    def _extract_single(self) -> List[str]:
        ...
        if self.config.require_problem or self.config.require_tool:
            ...
            if self.config.require_problem:
                if classification == "TOOL":
                    self.console.print(
                        "❌ Document classified as TOOL - skipping extraction (use --require-problem flag only for PROBLEM papers)"
                    )
                    return ["Document classified as TOOL - extraction skipped"]
                elif classification == "PROBLEM":
                    self.console.print(
                        "✅ Document classified as PROBLEM - proceeding with extraction"
                    )
                elif classification == "ERROR":
                    self.console.print(
                        "⚠️  Classification failed - proceeding with extraction anyway"
                    )
                else:
                    self.console.print(
                        f"⚠️  Unknown classification result '{classification}' - proceeding with extraction anyway"
                    )

            elif self.config.require_tool:
                if classification == "PROBLEM":
                    self.console.print(
                        "❌ Document classified as PROBLEM - skipping extraction (use --require-tool flag only for TOOL papers)"
                    )
                    return ["Document classified as PROBLEM - extraction skipped"]
                elif classification == "TOOL":
                    self.console.print(
                        "✅ Document classified as TOOL - proceeding with extraction"
                    )
                elif classification == "ERROR":
                    self.console.print(
                        "⚠️  Classification failed - proceeding with extraction anyway"
        ...
        self.console.print(
            f"📄 Processing {self.total_chunks} chunks from {self.config.input_file}"
        )

        if self.config.worker_count > 1:
            # Parallel processing
            self.console.print(
                f"🔄 Using {self.config.worker_count} workers for parallel processing"
            )
    ...
    def _process_chunks_sequential(self, chunks: List[str]) -> List[str]:
        ...
        with Progress(
            ...
            for idx, chunk in enumerate(chunks):
                progress.update(
                    task, description=f"Processing chunk {idx + 1}/{self.total_chunks}"
                )

                # Generate prompt for current mode
                prompt = self._get_prompt_for_mode(
                    self.config.mode, chunk, idx, self.total_chunks
                )
    ...
    def _process_chunks_parallel(self, chunks: List[str]) -> List[str]:
        ...
        chunk_data = [
            (idx, chunk, idx, self.total_chunks) for idx, chunk in enumerate(chunks)
        ]
        ...
        with Progress(
            ...
            with ThreadPoolExecutor(max_workers=self.config.worker_count) as executor:
                # Submit all tasks
                future_to_chunk = {
                    executor.submit(self._process_chunk_parallel, data): data
    ...
    def _extract_batch(self) -> List[str]:
        ...
        self.console.print(
            f"🚀 Starting batch extraction in {self.config.mode.value} mode"
        )
    ...
    def _process_files_sequential(self) -> List[str]:
        ...
        with Progress(
            ...
            files_task = progress.add_task(
                "Processing files", total=len(self.config.input_files)
            )

            for file_idx, file_path in enumerate(self.config.input_files):
                progress.update(
                    files_task, description=f"Processing {Path(file_path).name}"
                )
                ...
                if self.config.require_problem or self.config.require_tool:
                    ...
                    if self.config.require_problem and classification == "TOOL":
                        self.console.print(
                            f"⚠️  Skipping {Path(file_path).name} - classified as TOOL"
                        )
                        progress.advance(files_task)
                        continue
                    elif self.config.require_tool and classification == "PROBLEM":
                        self.console.print(
                            f"⚠️  Skipping {Path(file_path).name} - classified as PROBLEM"
                        )
                        progress.advance(files_task)
                        continue
                ...
                for idx, chunk in enumerate(chunks):
                    # Generate prompt for current mode
                    prompt = self._get_prompt_for_mode(
                        self.config.mode, chunk, idx, len(chunks)
                    )
...
def load_model_config(config_file: str, model_shortname: str) -> ModelConfig:
    ...
    try:
            ...
            if server["shortname"] == model_shortname:
                ...
                if openai_api_key_config == "${OPENAI_API_KEY}":
                    ...
                    if not openai_api_key:
                        console.print(
                            "❌ OpenAI API key is configured to use environment variable "
                    ...
                    console.print(
                        f"❌ 'openai_api_key' not specified for model {model_shortname}"
                    )
                    sys.exit(1)
        ...
        raise ValueError(
            f"Model '{model_shortname}' not found. Available models: {available_models}"
        )
...
Examples:
  ...
  python master_extract.py papers/ --mode workflow --model scout --workers 8 --output batch_results.txt
    ...
    parser.add_argument(
        "input_path", help="Input file or directory to process (.txt, .md, .pdf files)"
    )
    parser.add_argument(
        "--mode",
        type=str,
        ...
        default="workflow",
        help="Extraction mode (default: workflow)",
    )
    parser.add_argument(
        "--output",
        default="extraction_results.txt",
    ...
    parser.add_argument(
        "--temperature", type=float, default=0.0, help="API temperature (default: 0.0)"
    )
    parser.add_argument(
        "--character-limit",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Notation guidance file (default: notation.txt)",
    )
    parser.add_argument(
        "--rubric",
        default="rubric.txt",
        help="Rubric guidance file (default: rubric.txt)",
    )
    parser.add_argument(
        "--require-problem",
        action="store_true",
...
def main():
    ...
    if args.require_problem and args.require_tool:
        console.print(
            "❌ Error: --require-problem and --require-tool flags cannot be used together"
        )
        sys.exit(1)
    ...
    guidance_files = {"notation": args.notation, "rubric": args.rubric}
