...
class PDFValidator:
    ...
    def __init__(
        ...
        self.strict_mode = (
            strict_mode  # If True, use qpdf; if False, prioritize functional validation
        )
        ...
        if not any([self.qpdf_available, self.use_pypdf2, self.use_pymupdf]):
            raise RuntimeError(
                "No PDF validation tools available. Install qpdf, PyPDF2, or PyMuPDF."
            )
...
def validate_pdfs_parallel(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_path = {
            executor.submit(validator.validate, pdf_path): pdf_path
        ...
        if show_progress and HAS_TQDM:
            progress_bar = tqdm(
                total=len(pdf_files), desc="Validating PDFs", unit="file"
            )
        else:
            progress_bar = None
        ...
        for future in as_completed(future_to_path):
            ...
            if progress_bar:
                ...
                progress_bar.set_postfix(
                    {"Valid": valid_count, "Parsable": parsable_count}
                )
...
def sort_pdfs_by_parsability(
    results: List[ValidationResult], output_dir: str, copy_files: bool = True
) -> Tuple[int, int]:
    """
    ...
    for result in progress_bar:
        ...
        if HAS_TQDM and hasattr(progress_bar, "set_postfix"):
            progress_bar.set_postfix({"Good": good_count, "Bad": bad_count})
...
def create_sorting_report(
    results: List[ValidationResult], output_dir: str, good_count: int, bad_count: int
):
...
def main():
    ...
    parser.add_argument(
        "--show-invalid", action="store_true", help="Show details for invalid files"
    )
    parser.add_argument(
        "--sort-dir", help="Sort PDFs into GOOD/BAD directories based on parsability"
    )
    parser.add_argument(
        "--move-files",
        action="store_true",
    ...
    try:
        ...
        if len(pdf_files) == 1:
            ...
            results = validate_pdfs_parallel(
                pdf_files, args.max_workers, show_progress, strict_mode
            )
        ...
        if not args.summary_only:
            ...
            for result in results:
                ...
                print(
                    f"  Valid: {result.is_valid} | Parsable: {result.is_parsable} | "
        ...
        if args.sort_dir:
            ...
            good_count, bad_count = sort_pdfs_by_parsability(
                results, args.sort_dir, copy_files
            )
