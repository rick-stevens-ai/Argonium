...
def _evaluate_answer_with_retry(question, reference_answer, model_answer, config, question_format='auto'):
    """
    Wrapper for evaluate_answer with custom retry logic for JSON parsing issues.
    """
    # First attempt
    try:
        return _evaluate_answer_core(question, reference_answer, model_answer, config, question_format, retry_count=0)
    except Exception as e:
        if "JSON parsing failed, retrying once" in str(e):
            ...
            try:
                ...
                return _evaluate_answer_core(question, reference_answer, model_answer, config, question_format, retry_count=1)
...
def _evaluate_answer_core(question, reference_answer, model_answer, config, question_format='auto', retry_count=0):
    """
    ...
    if actual_format == 'mc':
        ...
        if ref_id and question:
            correct_option_content = extract_option_content(question, ref_id, ref_id_type)
        ...
        option_examples = "(1, 2, 3, etc.)" if option_type == "number" else "(A, B, C, etc.)"
        ...
        if ref_id:
            ...
            if correct_option_content:
                analysis_context += f"- Correct option content: \"{correct_option_content}\"\n"
        if model_id:
    ...
    try:
        ...
        try:
            ...
            if actual_format == 'mc':  # For multiple-choice questions
                ...
                if "correct_choice" in evaluation and "correct_letter" not in evaluation:
            ...
            print(f"Warning: Grader returned invalid JSON, attempting to parse response: {evaluation_text}")
            ...
            if score_match:
                ...
                if retry_count > 0:
                    print(f"Warning: Could not parse evaluation JSON after retry: {evaluation_text}")
                    ...
                    print(f"Warning: Could not parse evaluation JSON, will retry once: {evaluation_text}")
...
def evaluate_answer(question, reference_answer, model_answer, config, question_format='auto'):
    """
    Public interface for evaluating answers. Uses retry logic for JSON parsing issues.
    """
    return _evaluate_answer_with_retry(question, reference_answer, model_answer, config, question_format)

def process_question(item, model_config, grader_config, question_format='auto', verbose=False):
    """
    ...
    try:
        ...
        start_time = time.time()
        evaluation = evaluate_answer(question, reference_answer, model_answer, grader_config, question_format)
        ...
        if verbose:
            if format_type == 'mc':
                ...
                identifiers_match = model_letter == correct_letter if model_letter and correct_letter else None
                
                # If identifiers match but score is 0, or don't match but score is 1, there's an inconsistency
                if identifiers_match is not None and ((identifiers_match and score == 0) or (not identifiers_match and score == 1)):
                    print(f"WARNING: Score/{id_name} mismatch detected for Q{i}. Fixing...")
                    score = 1 if identifiers_match else 0
                    evaluation["score"] = score
                    
                print(f"Q{i} (MC) Result: {'✓' if score == 1 else '✗'} Score: {score}/1 " +
                      f"Confidence: {confidence:.2f} (Model chose {id_name}: {model_letter}, Correct {id_name}: {correct_letter})")
            else:  # QA format
                confidence = evaluation.get("confidence", 0)
                score_str = f"{score:.2f}" if isinstance(score, float) and score < 1 else f"{int(score)}/1"
                print(f"Q{i} (QA) Result: {'✓' if score >= 0.5 else '✗'} Score: {score_str} Confidence: {confidence:.2f}")
...
def parse_arguments():
    ...
    parser.add_argument('--output', help='Output JSON file for detailed results (default: results_<timestamp>.json)')
    parser.add_argument('--verbose', action='store_true', help='Print verbose output')
    parser.add_argument('--random', type=int, help='Randomly select N questions from the dataset (must be positive, default: use all questions)')
    parser.add_argument('--seed', type=int, help='Random seed for reproducible question selection (only used with --random)')
    parser.add_argument('--save-incorrect', action='store_true', help='Save incorrectly answered questions to a separate JSON file')
...
def main():
    ...
    if args.random:
        if args.random <= 0:
            ...
            print(f"Randomly selecting {args.random} questions from {original_count} total questions...")
            ...
            print(f"Requested {args.random} questions, but dataset only has {original_count}. Using all questions.")
    ...
    print(f"\nProcessing {len(items)} questions" + 
          (f" with {args.parallel} parallel workers..." if args.parallel > 1 else " sequentially..."))
    ...
    if args.parallel > 1:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.parallel) as executor:
            ...
            futures = [executor.submit(process_question, item, model_config, grader_config, question_format, args.verbose) 
                      for item in items]
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    if not args.verbose and not progress_bar:
                        ...
                        valid_results = [r for r in current_results if not r.get("skipped", False)]
                        if valid_results:
                            total_score = sum(r.get("score", 0) for r in valid_results)
                            accuracy = total_score / len(valid_results)
                            print(f"Completed {current_completed}/{total} questions | Running accuracy: {accuracy:.1%}", end="\r")
        ...
        for item in items:
            result = process_question(item, model_config, grader_config, question_format, args.verbose)
            results.append(result)
            completed += 1
            ...
            if not args.verbose:
                if valid_count > 0:
                    ...
                    print(f"Completed {completed}/{total} questions | Running accuracy: {accuracy:.1%}", end="\r")
                else:
    ...
    for result in mc_results:
        ...
        letters_match = model_letter == correct_letter if model_letter and correct_letter else None
        score = result.get("score", 0)
        
        # If letters match but score is 0, or don't match but score is 1, fix the inconsistency
        if letters_match is not None and ((letters_match and score == 0) or (not letters_match and score == 1)):
    ...
    if args.verbose:
        for result in valid_results:
            ...
            if format_type == 'mc':
                ...
                print(f"Result: {'✓' if score == 1 else '✗'} Score: {score}/1 Confidence: {confidence:.2f} " + 
                ...
                score_str = f"{score:.2f}" if isinstance(score, float) and score < 1 else f"{int(score)}/1"
                print(f"Result: {'✓' if score >= 0.5 else '✗'} Score: {score_str} Confidence: {confidence:.2f}")
    ...
    if all_scores:
        ...
        print(f"Overall accuracy: {overall_accuracy:.2%} ({sum(all_scores):.1f}/{len(all_scores)})")
    else:
    ...
    if mc_scores:
        ...
        print(f"Multiple-choice questions: {mc_accuracy:.2%} accuracy ({sum(mc_scores)}/{len(mc_scores)})")
        
        # Calculate average confidence for MC
        mc_confidences = [r.get("evaluation", {}).get("confidence", 0) for r in mc_results]
        avg_mc_confidence = sum(mc_confidences) / len(mc_confidences) if mc_confidences else 0
        print(f"Average MC confidence: {avg_mc_confidence:.2f}")
    
    # QA statistics
    if qa_scores:
        qa_accuracy = sum(qa_scores) / len(qa_scores)
        print(f"Free-form QA questions: {qa_accuracy:.2%} score ({sum(qa_scores):.1f}/{len(qa_scores)})")
        
        # Calculate average confidence for QA
        qa_confidences = [r.get("evaluation", {}).get("confidence", 0) for r in qa_results]
        ...
        if incorrect_results:
            ...
            incorrect_data = {
                "metadata": {
                    ...
                    "incorrect_rate": len(incorrect_results) / len(valid_results) if valid_results else 0,
            ...
            print(f"Saved {len(incorrect_results)} incorrect answers to: {incorrect_file}")
        else:
    ...
    avg_confidence = sum([r.get("evaluation", {}).get("confidence", 0) for r in valid_results]) / len(valid_results) if valid_results else 0
