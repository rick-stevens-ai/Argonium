#!/usr/bin/env python3
# make_v17.py - Question Generation Script with OpenAI Batch API Support
# 
# This version enhances make_v16.py by adding support for the OpenAI batch API,
# which can significantly improve performance when the server supports it.
# The script tests if the server supports batch API and automatically uses it
# when available, falling back to the standard API when not available.
...
_supports_batch_api = False  # Whether the OpenAI server supports batch API
_batch_queue = queue.Queue()  # Queue for batching API requests
_batch_lock = threading.Lock()  # Lock for updating the batch queue
_batch_results = {}  # Dictionary to store batch results

# Lock for updating shared counters
_counter_lock = threading.Lock()

# Batch processing settings
_batch_size = 10  # Maximum requests to batch together
_batch_wait_time = 0.1  # Maximum time to wait for batch to fill
_batch_processor_thread = None  # Thread for processing batches
_error_log_file = None  # File to log errors to
_max_error_threshold = 100  # Maximum number of errors before stopping the program
...
class TerminalUI:
    ...
    def _update_stats_pane(self):
        ...
        # Draw header
        self.top_pane.addstr(0, 0, " AMR Question Generation Progress ".center(self.top_pane.getmaxyx()[1], "="), 
        ...
        # Questions generated
        y += 1
        self.top_pane.addstr(y, 2, f"Questions Generated: ", curses.A_BOLD)
...
def process_batch_queue():
    """
    Process requests in the batch queue.
    This function runs in a background thread.
    """
    global _batch_queue, _batch_results, _batch_lock, _exit_requested, _supports_batch_api
    
    log_message("Starting batch processor thread", log_level="INFO")
    ...
    while not _exit_requested:
        try:
            ...
            # Try to collect up to batch_size requests or wait for batch_wait_time
            while (len(batch_requests) < _batch_size and 
                   time.time() - batch_collect_start < _batch_wait_time and 
                   not _exit_requested):
                
                try:
                    # Non-blocking get with timeout
                    request_id, request = _batch_queue.get(block=True, timeout=_batch_wait_time)
                    batch_requests.append(request)
                    batch_ids.append(request_id)
                    
                    # If we got one item and the queue is empty, we can stop waiting
            ...
            # If we have requests to process and the program is not shutting down
            if batch_requests and not _exit_requested:
                try:
                    # Make batch API request
                    batch_size = len(batch_requests)
                    log_message(f"Processing batch of {batch_size} requests", log_level="DEBUG")
                    
                    # Send the batch request
                    batch_response = openai.ChatCompletion.create(requests=batch_requests)
                    
                    # Process the responses and store them in the results dictionary
                    with _batch_lock:
                        for i, response in enumerate(batch_response.object_values):
                            request_id = batch_ids[i]
                            _batch_results[request_id] = {
                                "success": True,
                                "response": response
                            }
                    
                    log_message(f"Successfully processed batch of {batch_size} requests", log_level="DEBUG")
                    
                except Exception as e:
                    # If batch request fails, fall back to individual requests
                    log_message(f"Batch request failed: {e}. Falling back to individual requests.", log_level="WARNING")
                    ...
                    for i, request in enumerate(batch_requests):
                        ...
                        try:
                            ...
                            individual_response = openai.ChatCompletion.create(**request)
                            ...
                            log_message(f"Individual request {request_id} failed: {inner_e}", log_level="ERROR", error_type="api")
            
            # If no requests or exiting, sleep a bit to prevent CPU hogging
            elif not batch_requests:
                time.sleep(0.05)
                
        except Exception as e:
            log_message(f"Error in batch processor thread: {e}", log_level="ERROR", error_type="api")
            # Sleep a bit to prevent CPU hogging in case of recurring errors
            time.sleep(0.5)
    
    log_message("Batch processor thread shutting down", log_level="INFO")


def submit_to_batch_queue(request_params):
    """
    Submit a request to the batch queue and wait for the result.
    
    Args:
        request_params: The parameters for the API request
        
    Returns:
        The API response
    """
    global _batch_queue, _batch_results, _batch_lock, _supports_batch_api
    
    # Generate a unique request ID
    request_id = f"req_{int(time.time() * 1000)}_{random.randint(0, 1000000)}"
    
    # Submit the request to the queue
    _batch_queue.put((request_id, request_params))
    
    # Wait for the result
    while True:
        # Check if the result is available
        with _batch_lock:
            if request_id in _batch_results:
                result = _batch_results[request_id]
                del _batch_results[request_id]  # Clean up
                
                # Return the result or raise the error
                if result["success"]:
...
def start_batch_processor():
    """Start the batch processor thread if batch API is supported."""
    global _batch_processor_thread, _supports_batch_api
    
    if _supports_batch_api and _batch_processor_thread is None:
        _batch_processor_thread = threading.Thread(
            target=process_batch_queue,
            daemon=True,
            name="BatchProcessor"
        )
        _batch_processor_thread.start()
        log_message("Started batch processor thread", log_level="INFO")


def stop_batch_processor():
    """Stop the batch processor thread."""
    global _batch_processor_thread, _exit_requested
    
    # Set exit flag (the thread will exit on its own)
    _exit_requested = True
    
    # Wait for the thread to exit
    if _batch_processor_thread and _batch_processor_thread.is_alive():
        log_message("Waiting for batch processor thread to exit...", log_level="INFO")
        _batch_processor_thread.join(timeout=2.0)
        if _batch_processor_thread.is_alive():
            log_message("Batch processor thread did not exit cleanly", log_level="WARNING")
        else:
            log_message("Batch processor thread exited", log_level="INFO")


def batched_openai_completion(model: str, messages: list, **kwargs):
    """
    Make an OpenAI API call, using batch API if supported.
    ...
    # Use batch API if available, otherwise fall back to standard API
    if _supports_batch_api:
        return submit_to_batch_queue(request_params)
    else:
...
def shutdown_worker_pool():
    ...
    """
    global _worker_pool, _active_workers, _exit_requested, _supports_batch_api, _batch_processor_thread
    
    # First, shut down the batch processor if it's running
    if _supports_batch_api and _batch_processor_thread and _batch_processor_thread.is_alive():
        log_message("Shutting down batch processor...", log_level="WARNING")
...
def worker_process_chunk(chunk_id, chunk_text, model_name, question_type, num_answers, min_score):
    ...
    try:
        ...
        global _active_workers, _exit_requested, _supports_batch_api
        with _counter_lock:
            _active_workers += 1
...
def signal_handler(signum, frame):
    ...
    """
    global _exit_requested, terminal_ui, _worker_pool, _batch_processor_thread, _supports_batch_api
    ...
    # Shutdown the batch processor if running
    if _supports_batch_api and _batch_processor_thread and _batch_processor_thread.is_alive():
        log_message("Shutting down batch processor...", log_level="WARNING")
        stop_batch_processor()
...
def process_chunk(chunk_id: str, chunks_dir: str, model_name: str, 
    ...
    if terminal_ui and _use_split_screen:
        ...
        status_msg += "Multiple Choice" if question_type == QuestionType.MULTIPLE_CHOICE else "Free Form"
        terminal_ui.update_stats(
...
def generate_multiple_choice_qa_pairs(chunk_id: str, chunk_text: str, model_name: str, 
    ...
    user_message_2 = (
        ...
        f"with exactly {num_answers} answer choices labeled A through {chr(ord('A') + num_answers - 1)}.\n\n"
        f"Text:\n{augmented_chunk}\n\n"
        f"Requirements:\n"
        f"1. Create a challenging question that tests deep understanding.\n"
        f"2. Ensure there is EXACTLY ONE clearly correct answer.\n"
        f"3. Make the other choices plausible but clearly incorrect.\n"
        f"4. The question should focus on a concept or fact that is clearly stated or strongly implied in the text.\n"
        f"5. Number your answer choices from A to {chr(ord('A') + num_answers - 1)}.\n"
        f"6. Finally, indicate which letter (A-{chr(ord('A') + num_answers - 1)}) is the correct answer.\n"
        f"7. DO NOT provide explanations for why each answer is correct or incorrect.\n"
        f"8. IMPORTANT: The question must stand alone without referencing the source text. DO NOT include phrases like 'according to the text', 'in the passage', 'the author states', etc.\n\n"
        f"Your response must follow this format precisely: \n"
        f"QUESTION: <the question>\n"
        f"A: <first answer choice>\n"
        f"B: <second answer choice>\n"
        f"...\n"
    ...
    user_message_3 = (
        ...
        f"- Self-contained: Does the question stand alone without referring to 'the text', 'the passage', etc.?\n\n"
    ...
    # Extract question and answers from the model output
    ...
    correct_answer_match = re.search(r"CORRECT ANSWER:\s*([A-Z])", step2_output)
    correct_answer_letter = correct_answer_match.group(1) if correct_answer_match else "?"
    
    # Find the index of the correct answer
    if 'A' <= correct_answer_letter <= chr(ord('A') + num_answers - 1):
        correct_index = ord(correct_answer_letter) - ord('A')
    else:
        correct_index = 0  # Default to first answer if something went wrong
    
    # Format the question with embedded choices like in NAT-MC.json
    formatted_question = question + "\n\n"
    for i, choice in enumerate(answers):
        label = chr(ord('A') + i)  # A, B, C, ...
        choice_marker = "(*)" if label == correct_answer_letter else ""  # Mark correct answer with (*)
        formatted_question += f"{i+1}) {choice} {choice_marker}  \n"
    
    # Create detailed answer explanation
    answer_explanation = f"The correct answer is {correct_answer_letter}) {answers[correct_index]}. {critique}"
...
def generate_free_form_qa_pairs(chunk_id: str, chunk_text: str, model_name: str, 
    ...
    user_message_2 = (
        ...
        f"Text:\n{augmented_chunk}\n\n"
        f"Requirements:\n"
        f"1. Create a question that probes deep understanding rather than simple recall.\n"
        f"2. Ensure the question has a clear, unambiguous answer.\n"
        f"3. The question should focus on important concepts or facts from the text.\n"
        f"4. Make it specific enough that the answer would demonstrate real comprehension.\n"
        f"5. The question should be concise and clearly worded.\n"
        f"6. IMPORTANT: The question must stand alone without referencing the source text. DO NOT include phrases like 'according to the text', 'in the passage', 'the author states', etc.\n\n"
        f"Your response must follow this format precisely:\n"
        f"QUESTION: <the question>\n"
        f"ANSWER: <the complete, detailed answer>\n"
    )
...
class CheckpointManager:
    ...
    def force_save(self):
        ...
        try:
            ...
            log_message(f"Checkpoint saved: {processed_count} files, {chunks_count} chunks, {questions_count} questions")
...
def process_directory(input_dir: str, output_file: str, chunks_dir: str, model_name: str, 
    ...
    try:
        ...
        if terminal_ui and _use_split_screen:
            terminal_ui.update_stats(
                status_message=f"Complete! Saved {checkpoint_manager.get_questions_count()} questions to {output_file}"
            )
            update_global_stats()
...
def write_chunk_to_file(chunk_id: str, chunk_text: str, chunks_dir: str) -> str:
    ...
    try:
        ...
        log_message(f"Error writing chunk {chunk_id} to file: {e}", log_level="ERROR", error_type="file_processing")
        return None
...
def extract_text_from_pdf(file_path: str) -> str:
    ...
    try:
        ...
        log_message(f"Error extracting text from PDF {file_path}: {e}", log_level="ERROR", error_type="file_processing")
...
def extract_text_from_txt(file_path: str) -> str:
    ...
    try:
        ...
        log_message(f"Error extracting text from {file_path}: {e}", log_level="ERROR", error_type="file_processing")
        try:
            ...
            log_message(f"Error with fallback encoding for {file_path}: {e}", log_level="ERROR", error_type="file_processing")
            return ""
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description='Generate questions from text documents. V17 now supports OpenAI batch API for improved performance.')
    parser.add_argument('input_directory', help='Directory containing PDF, TXT, or MD files')
    parser.add_argument('--type', type=QuestionType, choices=list(QuestionType), default=QuestionType.FREE_FORM, 
                        help='Type of questions to generate: mc (multiple-choice) or qa (free-form)')
    ...
    parser.add_argument('--model', default='llama', help='Model shortname from model_servers.yaml to use')
    ...
    parser.add_argument('--error-threshold', type=int, default=100, help='Maximum number of errors of any type before stopping (default: 100)')
    return parser.parse_args()
    
    
def test_batch_api_support():
    """
    Test if the OpenAI server supports the batch API.
    
    Returns:
        bool: True if batch API is supported, False otherwise
    """
    global _supports_batch_api
    
    try:
        log_message("Testing if server supports batch API...", log_level="INFO")
        
        # Simple test to see if the server supports the batch API
        # This is a minimal request that should work if batch is supported
        test_messages = [
            {"role": "system", "content": "You are a helpful assistant."},
            {"role": "user", "content": "Say hello."}
        ]
        
        # Create a list of minimal messages for the batch request
        batch_request = [
            {"model": openai.default_model, "messages": test_messages}
        ]
        
        # Try to make a batch request
        openai.ChatCompletion.create(requests=batch_request)
        
        # If we get here, batch API is supported
        _supports_batch_api = True
        log_message("✅ Server supports batch API - will use for improved performance", log_level="INFO")
        return True
        
    except Exception as e:
        # Check if the error indicates batch API is not supported
        error_str = str(e)
        if "requests" in error_str and ("invalid" in error_str.lower() or "not supported" in error_str.lower()):
            log_message("ℹ️ Server does not support batch API - using standard API", log_level="INFO")
        else:
            log_message(f"❌ Error testing batch API: {e}", log_level="WARNING")
            log_message("Falling back to standard API", log_level="INFO")
        
        _supports_batch_api = False
        return False


def configure_apis(model_name: str) -> str:
    """
    ...
    try:
        with open("model_servers.yaml", "r") as f:
            servers_config = yaml.safe_load(f)
    except Exception as e:
        log_message(f"Error loading model_servers.yaml: {e}", log_level="ERROR")
        sys.exit(1)
    ...
    if not selected_server:
        log_message(f"Error: Model '{model_name}' not found in model_servers.yaml", log_level="ERROR")
    ...
    openai.default_model = actual_model_name  # Store for convenience
    
    # Set organization if provided
    if "org_id" in selected_server:
        openai.organization = selected_server["org_id"]
        
    log_message(f"Configured OpenAI API with base URL: {openai.api_base}")
    log_message(f"Using model shortname: {model_name}")
    log_message(f"Actual model identifier: {actual_model_name}")
    
    # Test if the server supports batch API
    test_batch_api_support()
...
def main():
    ...
    actual_model_name = configure_apis(args.model)
    
    # Start batch processor if the API supports it
    if _supports_batch_api:
